...
@section('centralize-content')
        ...
        <!-- /.login-logo -->
            ...
            <form>
                ...
                <div class="form-group {{ $errors->has('username') ? 'has-error' : '' }}">
                    <div>
                        {{ Form::text(fake_field('first_name'), old('first_name', null), ['class'=>'form-control', 'placeholder' => __('Full Name'),'data-cval-name' => 'The first name field','data-cval-rules' => 'required|escapeInput|alphaSpace']) }}
                ...
                <div class="form-group  {{ $errors->has('password') ? 'has-error' : '' }}">
                    <div>
                        {{ Form::password(fake_field('password'), ['class'=>'form-control', 'placeholder' => __('Password'),'data-cval-name' => 'The password field','data-cval-rules' => 'required|escapeInput|between:6,32|followedBy:'.fake_field('password_confirmation')]) }}
                ...
                <div  class="form-group {{ $errors->has('password_confirmation') ? 'has-error' : '' }}">
                    <div>
                        {{ Form::password(fake_field('password_confirmation'), ['class'=>'form-control', 'placeholder' => __('Confirm Password'),'data-cval-name' => 'The confirm password field','data-cval-rules' => 'required|escapeInput|between:6,32|follow:'.fake_field('password')]) }}
                        <span class="glyphicon glyphicon-lock form-control-feedback"></span>
                    </div>
                    <span class="validation-message cval-error"
                ...
                <div class="form-group {{ $errors->has('check_agreement') ? 'has-error' : '' }}">
                    <div>
                        <input type="checkbox" class="custom-control-input" id="form-checkbox" value="1" data-cval-name='The agreement field'
                            data-cval-rules='required'{{old('check_agreement') ? ' checked' : ''}}> </input>
                ...
                <button type="submit" class="btn btn-primary">{{ Form::submit(__('Create Account'), ['class'=>'btn btn-primary btn-flat btn-block form-submission-button']) }}</button>
                {{ Form::close() }}
            </form>
    ...
    <br>
    <p align="center">Already have an account? <a href="{{ route('login') }}">Sign in here</a></p>
@endsection
